...
import { LoginRequest } from '../vet-auth/login-request';
import { LoginResult } from '../vet-auth/login-result';
...
export class VetLoginComponent {
  ...
  constructor(
    ...
    private authService: VetAuthService, // Inject AuthService
    ...
    this.authService.login(loginRequest).subscribe({
      next: (result) => {
        ...
        if (result.success) {
          ...
          const redirectTo = this.activatedRoute.snapshot.queryParamMap.get('redirectTo') || '/vet/dashboard';
